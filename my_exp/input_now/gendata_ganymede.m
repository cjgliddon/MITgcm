...
% Ganymede: ~120 K (Squyres, Icarus, 1980); Callisto: 126 K (Squyres 1980)
% Titan: 94 K (Jennings et al., ApJ Lett., 2016). This is nearly constant!
Ts0=120; % K, ice surface temperature, used to compute conductive heat loss.
meridionalTs=1; % 0: constant Ts, 1: Ojakangas and Stevenson 1989 approximation, 3: Nadeau and McGehee 2018
% Ganymede: 0.2 deg; Callisto: 0.5 deg (Bills, Icarus, 2005)
% Since Titan's surface temp. is nearly constant this isn't so important.
obl=0; % depending on the scheme, the obliquity of the moon may be used to calculate Ts profile
...
%% domain
% Ganymede: seems very poorly constrained and sensitive to the salinity!
% (Vance et al. 2014). Depending on the temperature of the upper boundary,
% one obtains depth ranges of the upper layer + ocean from 50 km to 800 km.
% Lol. In a number of these cases one finds that the upper ice layer is of
% comparable thickness or even thicker than the ocean, which implies that
% the "thin-shell" approximations used in the tidal heating calculation are
% probably invalid. For an appropriately thin-shelled case, we might take
% the depth to be 84 km upper ice + 321 km ocean, corresponding to a 260-K
% upper interface temperature and a 10% by weight salinity.
% For Callisto: similar issue. Vance et al. (2018) give a suggestion of a
% 100-km upper layer and a 132-km ocean for a 10% salinity ocean.
% For Titan: this feels like the place where the thin-shell approximation
% might be most valid, with a ~400 km ocean and ~50 km ice shell for some
...
% TODO: need to understand exactly what the usePT flag does.

%% ice shell geometry
% Vance et al. (2018): 70 km thick ice shell
Hice0=70e3; Hice_P1=0e3; Hice_P2=0e3; Hice_P3=0; % 0.025H0, 0.3H0
...
%% heating terms
Htide0_portion=0; % portion of heat production in the ice shell, 1: 100% shell heating, 0: 100% core heating
...
PrescribeFreezing=0; % 1: prescribe freezing/melting rate using thin shell ice flow model (see Kang and Flierl 2020), 0: allow ice to respond to ocean heat transport
...
a0=2634.1e3; % m, radius
G=6.67e-11; % gravitational const
M=1.4819e23; % mass
rhoout=1000; % kg/m3, outer layer density
rhocore=3*M/(4*pi*(a0-Htot)^3)-rhoout*((a0/(a0-Htot))^3-1);    % core density inferred from total mass
...
hh=-Htot*hh;
if (bathyType==1)
    % add a cosine^2 variation in bottom depth
    deltaH = 30e3;
...
if restartTS==0 % prescribe analytical temp, salt IC profiles
     ...
     % dTnoise=delTemp0*0.01;
     dTnoise=0.01;
     dTn=rand([nx,ny]);
     dTn(:,ny/2+1:ny)=dTn(:,ny/2:-1:1);
     dTn=dTn-mean(mean(dTn,1).*wgt1);
     dTn=dTn(:);
     % ad-hoc change: noise added in middle of ocean rather than at boundary
