...
!*****************************************************************************************
    end module bspline_sub_mod
!*****************************************************************************************

!*****************************************************************************************
!program test_bspline_sub 
!
!    use bspline_kinds_module, only: wp, ip
!    use bspline_sub_module, only: db2val, db3val 
!    implicit none
!
...
!    integer, parameter :: oP = 6, oT = 6, om = 5
!
!    ! the knot sequences for each spline
!    real(wp)    :: P_Ih(nP_Ih + oP), P_II(nP_II + oP), P_III(nP_III + oP), P_V(nP_V + oP), P_VI(nP_VI + oP), P_salt(nP_salt + oP), &
!                   T_Ih(nT_Ih + oT), T_II(nT_II + oT), T_III(nT_III + oT), T_V(nT_V + oT), T_VI(nT_VI + oT), T_salt(nT_salt + oT), &
!                   m_salt(nm_salt + om)
!
...
!
!    ! "input/output" values
!    real(wp)    :: Ploc, Tloc, mloc
!    real(wp)    :: out
!
!    ! internal 
!    integer :: ios, i, j, k, iflag
!    integer :: u = 20
...
!    real(wp), dimension(oT)              :: w1_Ih    !! work array
!    real(wp), dimension(3_ip*oT)         :: w0_Ih    !! work array
...
!    OPEN (ACTION='read', FILE='csv/P_knots_Ih.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) P_Ih(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/P_knots_II.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) P_II(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/P_knots_III.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) P_III(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/P_knots_V.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) P_V(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/P_knots_VI.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) P_VI(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/P_knots_NaCl.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) P_salt(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/T_knots_Ih.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) T_Ih(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/T_knots_II.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) T_II(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/T_knots_III.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) T_III(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/T_knots_V.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) T_V(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/T_knots_VI.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) T_VI(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/T_knots_NaCl.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) T_salt(:)
!    CLOSE (u)
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/m_knots_NaCl.csv', IOSTAT=ios, UNIT=u)
!    read (u, *, IOSTAT=ios) m_salt(:)
!    CLOSE (u)
!    u = u + 1
!
!    ! load in the spline coefficients -------------------------------------------------------------
!    OPEN (ACTION='read', FILE='csv/bcoefs_Ih.csv', IOSTAT=ios, UNIT=u)
!    do i = 1, nP_Ih
...
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/bcoefs_II.csv', IOSTAT=ios, UNIT=u)
!    do i = 1, nP_II
...
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/bcoefs_III.csv', IOSTAT=ios, UNIT=u)
!    do i = 1, nP_III
...
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/bcoefs_V.csv', IOSTAT=ios, UNIT=u)
!    do i = 1, nP_V
...
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/bcoefs_VI.csv', IOSTAT=ios, UNIT=u)
!    do i = 1, nP_VI
...
!    u = u + 1
!
!    OPEN (ACTION='read', FILE='csv/bcoefs_NaCl.csv', IOSTAT=ios, UNIT=u)
!    do i = 1, nP_salt
...
!    mloc = 0.5
!    
!    call db2val(Ploc, Tloc, 0, 0, P_Ih, T_Ih, nP_Ih, nT_Ih, oP, oT, bcoefs_Ih, out, iflag, & 
!                inbvx, inbvy, iloy, w1_Ih, w0_Ih, .false.)
!    print *, "Gibbs energy of ice Ih at 100 MPa, 270 K:"
!    print *, out, "J/mol"
!    
!    call db3val(Ploc, Tloc, mloc, 0, 0, 0, P_salt, T_salt, m_salt, nP_salt, nT_salt, nm_salt, oP, oT, om, bcoefs_salt, out, iflag, &
!                jnbvx, jnbvy, jnbvz, jloy, jloz, w2_salt, w1_salt, w0_salt, .false.)
!    print *, "Gibbs energy of NaCl(aq) at 100 MPa, 270 K, 0.5 molal:"
!    print *, out, "J/mol"
!    
!    call db2val(Ploc, Tloc, 0, 1, P_Ih, T_Ih, nP_Ih, nT_Ih, oP, oT, bcoefs_Ih, out, iflag, & 
!                inbvx, inbvy, iloy, w1_Ih, w0_Ih, .false.)
!    print *, "Molar entropy of ice Ih at 100 MPa, 270 K:"
!    print *, -out, "J/mol*K"
!    
!    call db3val(Ploc, Tloc, mloc, 0, 1, 0, P_salt, T_salt, m_salt, nP_salt, nT_salt, nm_salt, oP, oT, om, bcoefs_salt, out, iflag, &
!                jnbvx, jnbvy, jnbvz, jloy, jloz, w2_salt, w1_salt, w0_salt, .false.)
!    print *, "Molar entropy of NaCl(aq) at 100 MPa, 270 K, 0.5 molal:"
!    print *, -out, "J/mol*K"
! 
!end program test_bspline_sub
