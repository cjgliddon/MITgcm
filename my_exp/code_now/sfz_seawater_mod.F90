...
!   This module contains routines for calculating thermodynamic properties of seawater up to 1 GPa
    ...
    ! REAL(8), DIMENSION(NT*NM) :: dummy                         ! a dufommy array for reading in data
    ! REAL(8), DIMENSION(NP-1, NT-1, NM-1, 4, 4, 4) :: G_spline_coeffs
...
!            PRINT *, 'Pressure in convert_PTS (Pa) : ', p
            p_MPa = p / 1.0E6
            T_K = T + 273.15
            M = S / mm_salt
!            PRINT *, 'Pressure in convert_PTS (MPa): ', p_MPa
            RETURN
...
!            LOGICAL :: coeffs_exist
!
!
!            CALL set_grid_params(P0, T0, M0, dP, dT, dM, NP, NT, NM)    ! set the parameters of the grid

!            INQUIRE( file='sf_G_coeffs.dat', exist=coeffs_exist)
!
!
!            IF (coeffs_exist .neqv. .true. ) THEN
...
!        END SUBROUTINE find_gibbs_sf
        ...
        SUBROUTINE find_rhosfz_scalar(p, T, S, spline_coeffs, rho)
        !   Calculates the density of seawater at the specified pressure, temperature, and salinity
            ...
            IF (params_set .neqv. .true. ) THEN
                CALL set_grid_params(P0, T0, M0, dP, dT, dM, NP, NT, NM)
            END IF
           
            CALL convert_PTS(p, T, S, pp, TT, MM)
!            PRINT *, 'pressure (MPa): ', pp
...
!            PRINT *, 'rho = :', rho
        ...
        SUBROUTINE find_alphasfz_scalar(p, T, S, spline_coeffs, alpha) 
...
!            PRINT *, 'alpha = :', alpha
        ...
        SUBROUTINE find_betasfz_scalar(p, T, S, spline_coeffs, beta)
        !   Calculates saline contraction coefficient as a function of p, T, S
            IMPLICIT NONE
            REAL(8), INTENT(IN) :: p, T, S
            REAL(8), DIMENSION(NP-1,NT-1,NM-1,4,4,4) :: spline_coeffs
            REAL(8), INTENT(OUT) :: beta
            REAL(8) :: pp, TT, MM, dgdp_int, d2gdpds_int
            ...
            CALL convert_PTS(p, T, S, pp, TT, MM) 
            CALL calc_spline(pp, TT, MM, 1, 0, 0, spline_coeffs, dgdp_int)
            CALL calc_spline(pp, TT, MM, 1, 0, 1, spline_coeffs, d2gdpds_int)
            beta = -(d2gdpds_int/dgdp_int)/mm_salt   ! convert back into dimensions of (g/kg)^-1
!            PRINT *, 'beta [(g/kg)^-1]=: ', beta
            RETURN 
